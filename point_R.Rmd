...
As it could be observed in our results, the final risk is way too high. The time of survival of the pathogens in human feaces is limited. Increasing the time between the excretion of the fertilizer and its application on the fields can therefore be used as a mitigation method. In order to assess its effectiveness, we take the calculation method implemented in Sch\önning et al., 2007. The formula used therefore is:

$$
H_i = M_{i-1} \left[1-\exp\left(-\frac{\ln(10)}{T_{90}}\right)\right]
$$

There are different inactivation rates in soil and in feaces. The mitigation measure can therefore be applied either to the soil or the feaces - mixing the excreta with soil will decay the pathogens in another pace than the the decay in the pure excreta. The values are taken from Sch\önning et al., 2007 
...
print_days = c(1,2,3,7,30,60,90, 180, 365, 550)
